...
include_once 'db.php';
    ...
    class Events extends DB{
        ...
        public function createEvent(){
            ...
            $query1 = $this->connect()->prepare('SELECT group_concat(title), langLvl, bookType, bookSubtype, numberClass, numberUnit, COUNT(*) Total
            FROM calendarEvent
            GROUP BY langLvl, bookSubtype, numberClass, numberUnit
            HAVING COUNT(*) > 1');
            ...
            $queryyy = $this->connect()->prepare('SELECT * FROM calendarevent WHERE title = :title AND langLvl = :langLvl AND bookType = :bookType AND bookSubtype = :bookSubtype AND numberClass = :numberClass AND numberUnit = :numberUnit');
            ...
            if($queryyy->rowCount()){
                ?>
				<div id="alert"><p>¡Ups! Recuerda que no puede duplicar. <a href="" onclick="location.reload()"><b>Reserva en otra hora.</b></a></p></div>
				<?php
            }else{
